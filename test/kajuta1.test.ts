...
describe('KAJUTA1', () => {
  ...
  it('reacts to the screen-shove (gspec=4): apologizes and marks it handled', () => {
    ...
    // (kajuta1.ts:197, URoom.pas:9378) and that would overwrite gspec back to 3. Send
    // every draw to its highest outcome so `random(100) < 1` cannot fire: the arming is
  ...
  it('re-arms the shove on the 1-in-100 draw, which pre-empts the gspec=4 reaction', () => {
    const s = kajuta1();
    s.room.gspec = 4;
    // The other side of the same draw: every draw at its lowest, so the arming fires.
    pinRandomLowest();
    KAJUTA1.prog(s);
