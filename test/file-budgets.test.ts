...
const BUDGETS: ReadonlyArray<readonly [path: string, maxLines: number]> = [
  ...
  // 1 646 -> 1 655 for the three hooks that make a failed room entry observable:
  // `loadNoteShown` and `loadNoteText` — two rather than one because the note's WORDING is
  // the thing under test, picked from the absent/failed taxonomy, and a probe that only
  // asked whether a note was up would pass just as happily on the sentence that blames the
  // player's connection for a 404 — plus `roomAudioPending`, the third hold a room entry
  // can be waiting on, which a probe otherwise cannot tell from the art hold.
  ['src/app/debugHooks.ts', 1655],
