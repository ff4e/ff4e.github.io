...
let graphics: GraphicsLevel =
  ((): GraphicsLevel => {
    ...
    // Default: the AI-upscaled tier, on every platform including the console. Each
    // element falls back to enhanced (and thence to classic) when it has no AI asset,
    // so this is safe even for anything unbuilt.
    //
    // The console must be registered as a *game* rather than an app for this to fit:
    // Xbox gives apps 1 GB and 45% of the GPU, but games 5 GB and the full GPU (see
    // xbox/README.md). At the app allowance the AI artwork can exhaust memory and get
    // the browser engine killed mid-play.
    return 'ai';
  })();
...
function drawMap(): void {
  ...
  if (useAi) {
    ...
    let drewOverlay = drawMapOverlays(overlay, true, plaque !== null);
    // The controller selection ring rides this shared overlay on the AI path: the
    // upscaled base map is drawn straight onto the hi-res ctx, so a ring blitted into
    // the native-resolution map buffer (as the faithful path does inside
    // worldMap.render) would never reach the screen. Same gate as that path — nodes
    // are hidden while the record panel is open.
    if (!panelOpen && selectedNode !== null) {
      worldMap.drawSelectionRing(overlay, selectedNode, tvMode, true);
      drewOverlay = true;
    }
    if (drewOverlay) {
      if (!mapOverlayCanvas) {
