...
import { initArtFailure } from './artFailure.js';
import {
  applyVolumeSettings,
  initPlayerSettings,
  musicLevel,
...
// The debug/test interface (window.__ff). Its 217 entries live in debugHooks.ts;
// what they need of the running game is handed over here, as getters, with setters
// for the four values probes deliberately write. Assigned to window HERE, at the
// end of boot, because tools/ui-lib.mjs waits on window.__ff as the signal that boot
// has completed.

//#region `window.__ff` host | anchors: debugHooks | The 99-member host the debug hooks read the game through: every member is a getter, and 4 of them also have a setter for the values probes deliberately write. State that has an owning module is imported there directly instead. The hooks themselves are in `debugHooks.ts`.
