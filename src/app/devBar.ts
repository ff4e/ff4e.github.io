...
export function initDevBar(h: DevBarHost): void {
  ...
  if (fitSelect) {
    ...
    el.addEventListener('change', () => {
      ...
      relayout(); // the layout carries the mode (StageLayout.mode) — re-derive it too
      wake();
    });
  }
  ...
  // effect immediately through the same layout and Options cleanup as device changes.
  if (touchSelect) {
