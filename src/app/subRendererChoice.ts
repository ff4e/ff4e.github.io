...
/**
 * Resolve a preference against the art tier and what the browser can do.
 *
 * `auto` is the shipped behaviour: the `ai` tier paints real DOM text whose motion is a
 * compositor `transform`, while `classic` and `enhanced` keep the canvas/baked path they
 * were tuned against. The two explicit values force one renderer everywhere — for
 * probes, for A/B-ing the two by eye, and for anyone the new path renders badly for.
 *
 ...
 */
export function resolveSubRenderer(pref: SubRendererPref, tier: string, supported: boolean): SubRenderer {
  if (!supported) return 'canvas';
  if (pref === 'canvas' || pref === 'dom') return pref;
  return tier === 'ai' ? 'dom' : 'canvas';
}
