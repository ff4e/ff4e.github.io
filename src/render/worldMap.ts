...
export class WorldMap {
  ...
  render(
    ...
    if (drawNodes && selectedNode !== null) this.drawSelectionRing(rgba, selectedNode, ringEmphasis);
    return rgba;
  }
   ...
   *
   * `onTransparent` targets the shared *overlay* buffer used by the AI (upscaled) map
   * path, which starts fully transparent and is composited over the hi-res art: there
   * the glow must be expressed as alpha, because there is no base map in the buffer to
   * blend against. On the faithful path the buffer already holds the opaque base map,
   * so the glow is blended in directly and every written pixel stays opaque.
   */
  drawSelectionRing(
