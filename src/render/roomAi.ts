 ...
 *    error, aborted connection, 5xx, a body that arrived truncated). Nothing was
 *    learned, so the caller MUST NOT cache it.
 *
 ...
 * null, every later entry joined the cached promise, and the room drew enhanced art
 * while the setting still said `ai` — with no way back short of reloading the app.
 *
...
export async function loadAiRoom(base: string, jmeno: string): Promise<AiRoom | null> {
  ...
  //
  // Hoisted out of the try so the catch can close whatever had already decoded — a
  // transient failure is retried on the next entry now, so leaking them is no longer a
  // one-off.
