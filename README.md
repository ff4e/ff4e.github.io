...
*33 regions over 2214 lines. Generated from the `//#region` markers in
...
| 1858–1872 | Boot | `initBoot`, `runBoot` | Hands `boot.ts` the one name it needs, then runs the boot sequence. The sequence itself — fonts, graphics, the save store, the sound packages, room 7, the first frame — is in that module. |
| 1873–2214 | `window.__ff` host | `debugHooks` | The 112-member host the debug hooks read the game through: 105 getters, plus 7 setters for the values probes deliberately write. State that has an owning module is imported there directly instead. The hooks themselves are in `debugHooks.ts`. |
