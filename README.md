...
*31 regions over 2020 lines. Generated from the `//#region` markers in
`src/app/main.ts` — edit a marker there, then run `npm run map:update`.*

| Lines | Region | Anchors — grep these | What lives here |
| --- | --- | --- | --- |
| 75–91 | Device gate | `isUnsupportedDevice`, `showUnsupportedNotice` | Phones are refused here, before any art is fetched — and before every other side effect in the file. The stage scaling and the tick constants that used to sit with it are in `stageGeometry.ts`. |
| 92–99 | Stage geometry wiring | `initStageGeometry` | Hands `stageGeometry.ts` its one name and takes the first stage measurement. The scaling itself is in that module. |
| 100–102 | Stage state wiring | `initStageState` | Assembles the stage and loads the persisted subtitle font. The state itself is in `stageState.ts`. |
| 103–105 | Loading UI wiring | `initLoadingUi` | Hands `loadingUi.ts` its one name and installs the boot-failure traps. The overlay, the fatal screen and relayout() are in that module. |
| 106–108 | Intro wiring | `initIntro` | Builds the intro player and probes for the AI movie variants. The movies and the subtitle overlay are in `introOverlay.ts`. |
| 109–114 | Screen & overlay state | `ui`, `hideAiCredits` | The mutable globals for panel/options/credits/map-info/help/leg-image moved to `screenState.ts` — import `ui` from there. Only the credits-overlay restore is left, because it is behaviour, not state. |
| 115–218 | Save store + cheats wiring | `openSaveStore`, `initCheats` | Opens `persist.ts` and hands `cheats.ts` its view of the game, after the gate. |
| 219–243 | Player settings wiring | `initPlayerSettings` | Hands `playerSettings.ts` its two names and loads the persisted options. Subtitle language and the volume buses are in that module. |
| 244–260 | Render settings wiring | `initRenderSettings` | Hands `renderSettings.ts` its one name and loads the four persisted choices. The tier/backend switches are in that module. |
| 261–294 | Art wiring | `initArt` | Hands `art.ts` its view of the game. The art loading itself is in that module. |
| 295–376 | Room launch wiring | `initRoomLaunch` | Hands `roomLaunch.ts` its view of the game. The parchment and the daRun state machine are in that module. |
| 377–423 | Audio & fish selection | `initAudio`, `hooks`, `peekAtPlayer`, `swapActive`, `selectFish` | Builds the AudioEngine (owned by `audioEngine.ts`), the fishing-hook easter egg, and switching which fish is active. The key/constant tables are in `keyTables.ts`. |
| 424–637 | Room construction | `buildRoom`, `setInfo`, `applySubFont`, `scriptTalk` | Turns parsed FFR data into a live `Room` + `StepEngine`, and refreshes the info line. Room *loading*, audio, movement and drawing are elsewhere. |
| 638–656 | Cutscene wiring | `initCutscene` | Hands `cutscene.ts` the five names it needs — the things a cutscene does to the game when it starts, ends or is skipped. The KUFRIK demo, the movies and the replay are in that module. |
| 657–669 | Room load wiring | `initRoomLoad` | Hands `roomLoad.ts` the three names it needs. Fetching a room, arming its voices and starting its music are in that module. |
| 670–685 | Movement wiring | `initMovement` | Hands `movement.ts` the four names it needs. The held-key state machine, the record replay and the room restart are in that module. |
| 686–764 | Save/load game | `saveGame`, `loadGame`, `saveExists`, `canSave`, `onWinBookkeeping` | In-room save slots and what happens on a win. |
| 765–777 | Panel wiring | `initPanel` | Hands `panel.ts` its three names. The side panel, the options sub-panel and the help overlay are in that module. |
| 778–793 | Map drawing wiring | `initMapDraw` | Hands `mapDraw.ts` the four names it needs, all of them the persisted record the map is a view of. The drawing is in that module. |
| 794–812 | Map navigation wiring | `initMapNav` | Hands `mapNav.ts` the five names it needs. Entering/leaving the map, the leg pages, the intro replay and the credits roll are in that module. |
| 813–1017 | Room entry & fish animation | `enterRoom`, `beginMapLaunch`, `panelAction`, `updateLipSync`, `fishFrameFor` | The map → room transition (incl. the launch parchment), panel button actions, and which sprite frame each fish shows. |
| 1018–1035 | Render plumbing wiring | `initGlPlumbing` | Hands `glPlumbing.ts` its view of the game. The compositors are in that module. |
| 1036–1045 | Frame painter wiring | `initFramePainter` | Hands `framePainter.ts` the two names it needs: which sprite frame each fish shows, and the hook system. The frame itself — all three tiers, both backends, the subtitle overlay — is in that module. |
| 1046–1061 | Logic tick wiring | `initLogicTick` | Hands `logicTick.ts` the four names it needs. The 80 ms step itself — script, engine, dialogue, death, screensaver — is in that module. |
| 1062–1095 | Frame pacing wiring | `initFramePacing` | Hands `framePacing.ts` its view of the game. The idle throttle, the wake rates and the perf HUD are in that module. |
| 1096–1111 | Render loop wiring | `initRenderLoop` | Hands `renderLoop.ts` the four names it needs: one logic step, and the three screen painters that still live here. The rAF callback is in that module. |
| 1112–1344 | Keyboard | `keydown / keyup listeners` | Every key binding, including cheats, dev keys and modal handling. |
| 1345–1653 | Pointer | `cellFromEvent`, `clickCell`, `dirToward`, `clickMapAt`, `panelCoords` | Fish selection and click-to-swim target (the pathfinding is in `stepEngine.ts`), map and panel hit-testing. |
| 1654–1663 | Dev bar wiring | `initDevBar` | Hands `devBar.ts` its two names. The room picker, the dev selects and the relayout watchers are in that module. |
| 1664–1678 | Boot | `initBoot`, `runBoot` | Hands `boot.ts` the one name it needs, then runs the boot sequence. The sequence itself — fonts, graphics, the save store, the sound packages, room 7, the first frame — is in that module. |
| 1679–2020 | `window.__ff` host | `debugHooks` | The 114-member host the debug hooks read the game through: getters, plus eight setters for the values probes deliberately write. State that has an owning module is imported there directly instead. The hooks themselves are in `debugHooks.ts`. |
