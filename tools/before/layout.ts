/**
 * `src/app/layout.ts` as it was BEFORE the layout rework — vendored verbatim from
 * `origin/main` at commit d3d70a6, and DELIBERATELY FROZEN. DEV ONLY.
 *
 * ── Why a frozen copy and not a live read of `main` ──────────────────────────
 * The lab's job is to show what a change did. A live read of `origin/main` answers that
 * only until the change merges, at which point it would be comparing the layout against
 * itself and would quietly become useless — exactly when someone would most want to check
 * that the old behaviour really is gone. Frozen at a named commit, this keeps answering
 * "what did it do before?" for as long as the question is worth asking.
 *
 * ── Do not edit, and nothing in `src/` may import it ─────────────────────────
 * Every line below is history, kept so `tools/layout-lab.html` can put the two layouts side
 * by side at the same viewport — so the two defects can be SEEN rather than taken on trust:
 *
 *  - a room drawn 266px tall into 214px of space at 669x280, 52px of the level off screen.
 *    `stageBoxWidth`/`stageBoxHeight` are `max(STAGE_W/H, avail/scale - margin)`, so on a
 *    small viewport the box is BIGGER than the space it sits in and the content fills it.
 *  - a reserve worth 21px per side at 1491x1114 and 0px at 1557. `STAGE_EDGE` is 12 NATIVE
 *    px inside that same box arithmetic, so it costs `12 x stageScale` on screen and stops
 *    being subtracted at all once the `max` picks its floor.
 *
 * Regenerate (only if the baseline should deliberately move):
 *   git show origin/main:src/app/layout.ts
 */
