 ...
 */
import { budget, tickSleep, waitFrames, waitRoom, withApp } from './ui-lib.mjs';
...
await withApp(
  async ({ p, expect }) => {
    await p.waitForFunction(() => window.__ff && typeof window.__ff.enterRoom === 'function', null, { timeout: budget(8000) });
    ...
    await p.waitForFunction(() => window.__ff.roomLoading(), null, { timeout: budget(10000) }).catch(() => {});
