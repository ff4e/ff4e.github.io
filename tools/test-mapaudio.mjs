...
await withApp(async ({ p, expect }) => {
  ...
  await p.waitForFunction(() => window.__ff.music() === 'menu', null, { timeout: budget(20000) }).catch(() => {});
  ...
  await p.waitForFunction(() => window.__ff.screen() === 'room' && window.__ff.music() === 'rybky04', null, { timeout: budget(20000) }).catch(() => {});
  expect((await p.evaluate(() => window.__ff.music())) === 'rybky04', 'room plays its own track');

  // Let PRVNI queue its opening dialogue.
  await p.waitForFunction(() => window.__ff.voicePlaying() || window.__ff.script()?.dialog, null, { timeout: budget(15000) }).catch(() => {});

  // Leave to the map: voices killed, dialogue queue cleared, menu music restored.
  await p.evaluate(() => window.__ff.showMap());
  await p
    .waitForFunction(() => window.__ff.screen() === 'map' && window.__ff.music() === 'menu', null, { timeout: budget(10000) })
    .catch(() => {});
