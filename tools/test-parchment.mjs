 ...
 * mapa-0_ai, 9.77 against mapa-1_ai), which is the x4 form of the 0.97/21.26 the native art
 * measures, and pins the rectangle's placement as well as the layer it belongs over.
 *
 ...
 * standing delay because the probe reboots twice and only three of its entries want it.
 *
 ...
 * still explain themselves; 4 the ai tier's seam. The other half of "do not make the
 * player wait" — the ai world map being fetched during the intro rather than on arrival —
 * needs a first-run boot, so it lives in test-intro.mjs with the rest of that path.
 */
 ...
 * the DARK map layer baked into its border (0.97 against mapa-0 and 21.26 against mapa-1,
 * measured on the native art). So the band must agree with mapa-0_ai and disagree with
...
await withApp(
  async ({ p, expect }) => {
    ...
    console.log('4. the ai tier draws the upscaled parchment, seamlessly');
