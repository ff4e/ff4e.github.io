...
await withApp(async ({ p, expect }) => {
  ...
  await p.waitForFunction(() => window.__ff.screen() === 'room' && window.__ff.count() > 0 && !window.__ff.state().won && !window.__ff.state().venku.little && !window.__ff.state().venku.big, null, { timeout: budget(40000) });
  ...
  await p.waitForFunction(() => window.__ff.screen() === 'room' && window.__ff.count() > 0 && !window.__ff.state().won && !window.__ff.state().venku.little && !window.__ff.state().venku.big, null, { timeout: budget(40000) });
  await winCurrentRoom();
  await p.waitForFunction(() => window.__ff.screen() === 'map', null, { timeout: budget(6000) });
  ...
  await p.waitForFunction(() => window.__ff.mapInfoRoom() === 19, null, { timeout: budget(3000) });
  ...
  await p.keyboard.press('Space');
  await p.waitForFunction(() => window.__ff.replayActive(), null, { timeout: budget(6000) });
