...
await p.addInitScript(() => {
  try {
    ...
    // Pin the tier this probe asserts on: the default is `ai`, whose art this
    // probe never uses but would still download and wait for.
...
await p.waitForFunction(() => window.__ff.enhancedActive(), null, { timeout: budget(10000) }).catch(() => {});
...
for (let num = 1; num <= 72; num++) {
  try {
    ...
    await p.waitForFunction(() => window.__ff.enhancedLoaded() || window.__ff.count() > 40, null, { timeout: budget(6000) }).catch(() => {});
...
await p.waitForFunction(() => window.__ff.screen() === 'room' && window.__ff.enhancedActive(), null, { timeout: budget(10000) });
...
try {
  await p.waitForFunction((h) => window.__ff.roomBgFrameHash('enhanced') !== h, wreckBefore, { timeout: budget(8000), polling: 50 });
} catch (e) {
