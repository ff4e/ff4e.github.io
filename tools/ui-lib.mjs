...
export async function gotoApp(p, query = '') {
  const port = process.env.FF_UI_PORT ?? '5173';
  // `query` (e.g. 'tv') lets a probe boot the app in a URL-gated mode such as TV/console
  // mode; omitted, the app loads exactly as the plain web build does.
  const q = query ? (query.startsWith('?') ? query : `?${query}`) : '';
  await p.goto(`http://127.0.0.1:${port}/${q}`, { waitUntil: 'domcontentloaded' });
  await appReady(p);
}
