...
- **`tools/`** — the CLIs and harnesses. `dump-ffr.ts` / `render-room.ts` are the M0/M1
  ...
  The **layout lab** is a group of its own: `layout-lab.html` draws the game's layout at any
  viewport with rooms as coloured rectangles and the numbers beside the picture,
